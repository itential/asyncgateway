...
class Scanner:
    ...
    _default_patterns: dict[str, re.Pattern] | None = None
        ...
        if Scanner._default_patterns is None:
            ...
            for name, pattern_str in patterns_to_compile.items():
                ...
                Scanner._default_redactions[name] = (
                    lambda _, n=name: f"[REDACTED_{n.upper()}]"  # type: ignore[misc]
                )

        # Copy pre-compiled patterns to instance
        assert Scanner._default_patterns is not None
    ...
    def scan_and_redact(self, text: str) -> str:
        ...
        for pattern_name, pattern in self._patterns.items():
            ...
            result = pattern.sub(
                lambda match, func=redaction_func: func(match.group(0)),  # type: ignore[misc]
                result,
            )
